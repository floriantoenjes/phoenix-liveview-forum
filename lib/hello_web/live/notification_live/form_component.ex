defmodule HelloWeb.NotificationLive.FormComponent do
  ...
  def update(%{notification: notification} = assigns, socket) do
    changeset = Forum.change_notification(notification)
  ...
  def handle_event("validate", %{"notification" => notification_params}, socket) do
    changeset =
      ...
      |> Forum.change_notification(notification_params)
  ...
  defp save_notification(socket, :edit, notification_params) do
    case Forum.update_notification(socket.assigns.notification, notification_params) do
      {:ok, _notification} ->
        {:noreply,
         socket
  ...
  defp save_notification(socket, :new, notification_params) do
    case Forum.create_notification(notification_params) do
      {:ok, _notification} ->
        {:noreply,
         socket
